...
bool Geometry::IsPointInLine(const Point2d& p, const Point2d& s, const Point2d& e)
{
...
std::vector<Point2d> Geometry::CalSections(const Point2d& a, const Point2d& b, unsigned int n)
{
  ...
  if (n == 0) {
    RRLOG_ERROR("n can't be 0");
    return sections;
  }

  sections.emplace_back(a);
  if (a.Equals(b)) {
    RRLOG_WARN("a is equals to b");
    return sections;
  }
...
bool Geometry::CalPointFromLineWithDistance(const Point2d& a, const Point2d& b, double d, Point2d& p)
{
  ...
  if (d < 0.0) {
    RRLOG_ERROR("d must be greater than 0.0");
    return false;
  }
